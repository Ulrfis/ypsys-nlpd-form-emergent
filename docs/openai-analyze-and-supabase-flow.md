...
3. **OpenAI** renvoie un JSON v3 : `teaser`, `lead_temperature`, `score_100`, `severity_band`, `top_issues`, `result_summary`, `result_focus_points`, `email_user`, `email_sales`.
...
Les **instructions complètes** de l'assistant (version active) sont dans **[docs/assistant-prompt-nlpd-v2-score100.md](assistant-prompt-nlpd-v2-score100.md)**. À copier dans la configuration de l'assistant OpenAI.
...
- **score_100** : score normalisé sur 100.
- **severity_band** : niveau global (`critical`, `vigilance`, `good`).
...
4. **Recopier les instructions (version active v2)**  
   Remplacer entièrement les Instructions de l’assistant par le contenu de [docs/assistant-prompt-nlpd-v2-score100.md](assistant-prompt-nlpd-v2-score100.md) (à partir de « Tu es un assistant expert… »). Enregistrer, refaire un test depuis l’app, puis revérifier les logs Railway.
