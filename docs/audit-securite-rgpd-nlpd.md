...
**Périmètre** : Frontend (React), Backend (FastAPI), Supabase, OpenAI

---

## 1. Résumé exécutif

L’application collecte des **données personnelles** (nom, prénom, email, entreprise, réponses au questionnaire) pour fournir un diagnostic de conformité nLPD et envoyer un rapport par email. Les données sont envoyées à **OpenAI** (traitement IA) et stockées dans **Supabase** (EU possible). Aucun cookie ni outil d’analytics (ex. PostHog) n’est utilisé sur le formulaire.

**Verdict** : Plusieurs points sont **conformes** (consentement explicite, clé API côté serveur, RLS Supabase). Les **actions prioritaires P0** ont été mises en œuvre (API backend protégée par `X-API-Key`, politique de confidentialité en place avec lien depuis le formulaire, logs debug sanitisés). PostHog et le bandeau cookies ont été retirés ; pas de cookies sur le formulaire. Il reste à documenter/encadrer les transferts US (OpenAI) dans la politique et à traiter les points P2 si besoin (consentement granulaire, sous-traitants).
...
(Aucun flux vers un outil d’analytics ou de cookies sur le formulaire.)
...
| **Art. 28 (sous-traitant)** | Pas de mention de sous-traitants (OpenAI, Supabase, hébergeur) ni de garanties (clauses type ou équivalent). | Responsabilité du responsable de traitement, contrôles CNIL. |
| **Art. 44 et s. (transferts)** | Données envoyées à OpenAI (US). Pas de mécanisme explicite (clauses types, DPA, etc.) ni d’information dans la politique. | Transferts illicites au sens RGPD. |
...
- **Transferts à l’étranger (art. 16 nLPD)** : OpenAI situé aux États-Unis. La Suisse exige un niveau de protection adéquat ou des garanties appropriées (clauses types, DPA). À documenter et à mentionner dans la politique.
...
### 5.5 Analytics et cookies (état : non utilisés)

- **PostHog retiré** : Aucun outil d’analytics (PostHog ou autre) n’est utilisé sur le formulaire.
- **Pas de bandeau cookies** : Le formulaire n’utilise pas de cookies ; aucun bandeau de consentement n’est affiché.
...
| **P1** | Transferts US (OpenAI) non documentés / non encadrés | Documenter, DPA / clauses types, mention dans la politique | À faire |
| — | PostHog / cookies | Retirés ; pas d’analytics ni de cookies sur le formulaire | ✅ N/A |
...
2. **Politique de confidentialité**  
   ...
   - Y inclure : responsable du traitement, finalités, données collectées, durée de conservation, destinataires (Supabase, OpenAI, Dreamlit si utilisé), transferts hors UE/CH et garanties, droits (accès, rectification, effacement, portabilité, opposition, réclamation auprès de l’autorité).  
...
3. **Transferts internationaux**  
   - Vérifier les contrats / DPA avec OpenAI pour clauses types ou garanties équivalentes.  
...
3. **PostHog et bandeau cookies retirés** : PostHog et le bandeau de consentement cookies ont été supprimés. Le formulaire n’utilise aucun cookie ni outil d’analytics.
